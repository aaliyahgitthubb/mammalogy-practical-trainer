...
function syncCardEditorDraft(){
  ...
  state.editor.front=$('cardFront')?.value||'';
  state.editor.back=$('cardBack')?.value||'';
  state.editor.tags=selectedEditorTags();
}
function cardEditorSpeciesChanged(id){syncCardEditorDraft();state.editor.speciesId=id;state.editor.imageId='';state.editor.imagePlacement='none';render()}
function selectEditorImage(id){syncCardEditorDraft();state.editor.imageId=id;state.editor.imagePlacement='none';render()}
function setEditorImagePlacement(value){syncCardEditorDraft();state.editor.imagePlacement=value;render()}
function editorSpeciesImages(){const sp=speciesById(state.editor?.speciesId);return sp?allImages(sp):[]}
function cardImage(c){if(!c?.imageId)return null;const sp=speciesById(c.speciesId);return sp?allImages(sp).find(i=>i.id===c.imageId)||null:null}
function cardImageMarkup(c,extraClass=''){const im=cardImage(c);if(!im)return '';return `<div class="flash-image ${extraClass}"><img src="${esc(im.data)}" alt="${esc(speciesById(c.speciesId)?.common||'Species image')}" onclick="event.stopPropagation();openLightbox('${esc(im.data)}')"></div>`}
function flashDeckCards(cards){const deck=[];cards.forEach(c=>{if(c.imagePlacement==='separate'&&c.imageId)deck.push({...c,__imageOnly:true});deck.push(c)});return deck}
...
function studyNext(){const cards=flashDeckCards(filteredStudyCards());if(cards.length){state.studyIndex=(state.studyIndex+1)%cards.length;state.studyFlipped=false;render()}}
function studyPrev(){const cards=flashDeckCards(filteredStudyCards());if(cards.length){state.studyIndex=(state.studyIndex-1+cards.length)%cards.length;state.studyFlipped=false;render()}}
function shuffleStudyCards(){const cards=flashDeckCards(filteredStudyCards());if(cards.length){state.studyIndex=Math.floor(Math.random()*cards.length);state.studyFlipped=false;render()}}
...
function renderCardEditor(app){
  ...
  const imagePreview=selectedImg?`<div class="preview-image-wrap"><img src="${esc(selectedImg.data)}" alt="Selected species image"></div>`:'';
  const previewFront=placement==='front'&&selectedImg?imagePreview+`<div>${esc(e.front||'Your question will appear here.')}</div>`:esc(e.front||'Your question will appear here.');
  const previewBack=placement==='back'&&selectedImg?`<div>${esc(e.back||'Your answer will appear here.')}</div>${imagePreview}`:esc(e.back||'Your answer will appear here.');
  ...
  app.innerHTML=shell(e.id?'Edit Study Card':'Create Study Card',`
    <div class="editor-layout"><section class="panel editor-form"><div class="form-grid"><label>Species<select id="cardSpecies">${speciesOptions}</select></label><label>Category<select id="cardCategory">${catOptions}</select></label></div>
      <div id="customCategoryWrap" class="custom-field" style="display:${isCustomCat?'block':'none'}"><label>Custom category<input id="customCategory" value="${esc(e.customCategory||(!cats.includes(e.category||'General')?e.category:'')||'')}" placeholder="e.g., Exam Review, Measurements, Comparison"></label></div>
      ...
      <div class="flash-image-editor panel"><h3>🖼️ Image from Species Library</h3><label class="check-row"><input id="includeCardImage" type="checkbox" ${e.imageId?'checked':''}> <span>Include an image from this species' library</span></label>
        ${e.imageId?`<div class="image-placement"><strong>Image placement:</strong><label><input type="radio" name="imagePlacement" value="none" ${placement==='none'?'checked':''}> No image</label><label><input type="radio" name="imagePlacement" value="separate" ${placement==='separate'?'checked':''}> Separate image card</label><label><input type="radio" name="imagePlacement" value="front" ${placement==='front'?'checked':''}> Front of card</label><label><input type="radio" name="imagePlacement" value="back" ${placement==='back'?'checked':''}> Back of card</label></div><div class="image-choice-grid">${imageChoices}</div>`:'<p class="muted">Check the box to choose an image from this species library. Image placement defaults to <b>No image</b>.</p>'}
      </div>
      <div class="tag-editor"><div class="tag-editor-title">Tags</div><div class="tag-input-row"><input id="tagInput" list="tagSuggestions" placeholder="Type a tag, then click Add (or press Enter)"><datalist id="tagSuggestions">${suggested.map(t=>`<option value="${esc(t)}">`).join('')}</datalist><button type="button" onclick="addEditorTag()">Add tag</button></div><div id="editorTags" class="tag-chip-list">${tags.map(t=>`<span class="tag-chip" data-tag="${esc(t)}">${esc(t)} <button type="button" aria-label="Remove ${esc(t)}" onclick="removeEditorTag(this)">×</button></span>`).join('')}</div><div class="muted">Use tags to organize cards such as <b>skull</b>, <b>dentition</b>, <b>family</b>, <b>exam</b>, or any custom label you want. Tags are saved with the card.</div></div>
      <div id="editorError" class="editor-error"></div><div class="button-row"><button class="primary" onclick="submitCardEditor()">Save Card</button><button onclick="cancelCardEditor()">Cancel</button></div>
    </section><section class="panel live-preview"><h2>Full Card Preview</h2><div class="preview-card"><div class="preview-label">FRONT</div><div id="previewFront" class="preview-text">${previewFront}</div><div class="preview-divider"></div><div class="preview-label">BACK</div><div id="previewBack" class="preview-text">${previewBack}</div><div class="preview-species"><span class="preview-base-species">${esc(speciesById(e.speciesId)?.common||'')} ·</span> <span id="previewCategory">${esc(e.category||'General')}</span><div id="previewTags" class="tag-row">${tags.map(t=>`<span class="tag secondary">${esc(t)}</span>`).join('')}</div></div></div><p class="muted">The selected species image is stored with this card. Existing cards remain unchanged and default to no image.</p></section></div>
    <section class="panel question-bank-panel"><div class="section-head"><div><h2>📚 Flashcard Question Bank</h2><p class="muted">All of your existing flashcards are shown here while you create or edit a card.</p></div><strong id="questionBankCount">${bankCards.length} card${bankCards.length===1?'':'s'}</strong></div><div class="question-bank-filters"><input id="questionBankSearch" value="${esc(state.cardBankQuery||'')}" placeholder="Search questions, answers, categories, or tags..." oninput="state.cardBankQuery=this.value;filterCardBank()"><select id="questionBankSpecies" onchange="state.cardBankSpecies=this.value;filterCardBank()">${bankSpeciesOptions}</select></div><div id="questionBankList" class="question-bank-list">${bankRows}</div></section>`);
  ...
  $('cardSpecies')?.addEventListener('change',e=>cardEditorSpeciesChanged(e.target.value));
  $('includeCardImage')?.addEventListener('change',e=>{syncCardEditorDraft();if(e.target.checked){const imgsNow=editorSpeciesImages();state.editor.imageId=state.editor.imageId&&imgsNow.some(x=>x.id===state.editor.imageId)?state.editor.imageId:(imgsNow[0]?.id||'')}else{state.editor.imageId='';state.editor.imagePlacement='none'};render()});
  document.querySelectorAll('input[name="imagePlacement"]').forEach(r=>r.addEventListener('change',e=>setEditorImagePlacement(e.target.value)));
}
