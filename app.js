...
function googlePhotosClientId(){return loadSettings().googlePhotosClientId||''}
...
async function importGooglePhotosToSpecies(){const sp=selectedSpecies();if(!sp){alert('Add a species first.');return}try{const token=await getGooglePhotosToken();const session=await googlePhotosRequest('https://photospicker.googleapis.com/v1/sessions',token,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({})});if(!session.id||!session.pickerUri)throw new Error('Google Photos did not return a picker session.');const picker=window.open(session.pickerUri+'/autoclose','_blank','noopener,noreferrer');if(!picker)alert('Your browser blocked the Google Photos window. Allow pop-ups for this site and try again.');const deadline=Date.now()+Math.min(Number(session.pollingConfig?.timeoutIn||600)*1000,10*60*1000);let ready=false;while(Date.now()<deadline){await new Promise(r=>setTimeout(r,Math.max(1000,Number(session.pollingConfig?.pollInterval||3)*1000)));const status=await googlePhotosRequest(`https://photospicker.googleapis.com/v1/sessions/${encodeURIComponent(session.id)}`,token);if(status.mediaItemsSet){ready=true;break}}if(!ready)throw new Error('Google Photos selection timed out.');let pageToken='',items=[];do{const u=new URL('https://photospicker.googleapis.com/v1/mediaItems');u.searchParams.set('sessionId',session.id);if(pageToken)u.searchParams.set('pageToken',pageToken);const page=await googlePhotosRequest(u.toString(),token);items=items.concat(page.mediaItems||[]);pageToken=page.nextPageToken||''}while(pageToken);let added=0;for(const item of items){const mf=item.mediaFile||{};if(!mf.baseUrl||!String(mf.mimeType||'').startsWith('image/'))continue;const rr=await fetch(mf.baseUrl+'=w2048-h2048',{headers:{Authorization:`Bearer ${token}`}});if(!rr.ok)continue;const blob=await rr.blob();if(!blob.type.startsWith('image/'))continue;const dataUrl=await blobToDataUrl(blob);await putCustomImage({id:`google-photos-${item.id}`,speciesId:sp.id,name:item.mediaMetadata?.creationTime?`Google Photos — ${new Date(item.mediaMetadata.creationTime).toLocaleDateString()}`:'Google Photos image',dataUrl,viewTypes:['mixed'],createdAt:Date.now(),source:'google-photos',sourceId:item.id});added++}await refreshStudyData();render();alert(added?`Imported ${added} image${added===1?'':'s'} from Google Photos into ${sp.common}.`:'No image photos were selected.')}catch(e){alert(e?.message||'Google Photos import failed.')}}
...
function renderQuiz(app){
  ...
  app.innerHTML=shell(`${state.mystery?'Mystery Specimen':modeLabel()} — Question ${state.session.q} of ${state.length}`,`
    ...
    <div class="quiz-card">
      <div class="image-wrap ${imageClass}" style="--quiz-mask-height:${maskHeight}%"><img src="${esc(imageSrc)}" alt="${esc(s.common)} specimen" ${labelMode==='show'?`onclick="openLightbox('${esc(imageSrc)}')"`:''}></div>
      <div class="quiz-side">
...
Object.assign(window,{
  ...
  handlePaste,handleFiles,saveImageFile,removeCustomImage,setImageTag,handleNewSpeciesFiles,removePendingSpeciesImage,importGooglePhotosToSpecies,saveGooglePhotosSettings,
